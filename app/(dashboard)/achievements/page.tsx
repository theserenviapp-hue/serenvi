...
export default function AchievementsPage() {
  ...
  const icons: Record<string, string> = {
    'Influencer': '***', 'Master': '**', 'Legend': '*', 'Icon': '#',
    'Titan': '!', 'Global Leader': '@', 'World Leader': '^', 'Empire Leader': '~', 'Global Icon': '+'
  };
  ...
  const fetchAchievements = async () => {
    try {
      ...
      const response = await fetch(`${BACKEND_URL}/achievements/progress`, {
        credentials: 'include',
      });
      if (response.ok) {
  ...
  const claimAchievement = async (rankName: string) => {
    ...
    try {
      ...
      const response = await fetch(`${BACKEND_URL}/achievements/claim/${rankName}`, {
        method: 'POST',
        credentials: 'include',
      });
      if (response.ok) {
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">
            Current rank: <span className="font-bold text-blue-600">{currentRank}</span>
          </p>
          <p className="text-gray-600">Personal Sales: ₹{personalSales.toLocaleString()}</p>
        </div>
          ...
          {achievementProgress.map((achievement, idx) => (
            <Card
              ...
              className={`transition border-2 flex flex-col h-full ${
                achievement.claimed ? 'border-green-500 bg-green-50'
                  : achievement.isUnlocked ? 'border-yellow-500 bg-yellow-50'
                  : 'border-gray-300 opacity-75 bg-white'
              }`}
            >
              <div className="flex justify-between items-start mb-4">
                <div className="text-4xl">{icons[achievement.rank] || '-'}</div>
                {achievement.claimed && <span className="text-green-600 font-bold text-xs bg-green-200 px-2 py-1 rounded">CLAIMED</span>}
                {achievement.isUnlocked && !achievement.claimed && <span className="text-yellow-600 font-bold text-xs bg-yellow-200 px-2 py-1 rounded">UNLOCKED</span>}
              </div>
              ...
              <div className="space-y-3 flex-1">
                <div>
                  ...
                  <p className="text-lg font-semibold text-gray-800">₹{achievement.salesTarget.toLocaleString()}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Reward</p>
                  <p className="text-lg font-semibold text-blue-600">₹{achievement.rewardAmount.toLocaleString()}</p>
                </div>
                <div>
                  <div className="w-full bg-gray-300 rounded-full h-2 overflow-hidden">
                    <div
                      className={`h-2 rounded-full transition-all ${achievement.claimed ? 'bg-green-600' : achievement.isUnlocked ? 'bg-yellow-500' : 'bg-blue-600'}`}
                      style={{ width: `${achievement.progressPercent}%` }}
                    />
                  </div>
                  <p className="text-xs text-gray-600 mt-1">{achievement.progressPercent}%</p>
                </div>
              </div>
              {achievement.isUnlocked && !achievement.claimed && (
                <Button onClick={() => claimAchievement(achievement.rank)} disabled={claiming === achievement.rank} className="w-full mt-4 bg-yellow-600 hover:bg-yellow-700">
                  {claiming === achievement.rank ? 'Claiming...' : 'Claim Reward'}
                </Button>
              )}
              {achievement.claimed && <Button disabled className="w-full mt-4 bg-green-600">Claimed</Button>}
              {!achievement.isUnlocked && (
                <p className="text-xs text-gray-500 text-center mt-4">
                  Need ₹{(achievement.salesTarget - achievement.personalSalesMade).toLocaleString()} more
                </p>
              )}
            </Card>
          ))}
